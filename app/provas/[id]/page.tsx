...
const PRINT_STYLES = `
  @media print {
    ...
    .printable-pill .overflow-y-auto { 
...
export default function ExamPreviewPage() {
  ...
  async function generateReviewPill(force = false) {
    if (pillData && !force) return // Se já tem pílula e não é force, não faz nada
    setPillLoading(true)
    try {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (examData?.review_pill) {
        setPillData(examData.review_pill)
      }
      setLoading(false)
    }
    loadData()
  ...
  return (
    <div className="max-w-[1400px] mx-auto animate-fade-in">
      ...
      {/* Modal da Pílula de Revisão (Fora do fluxo de print) */}
      {pillData && (
          ...
          <div className="bg-white w-full max-w-xl rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 printable-pill">
            <div className="bg-[#4F46E5] p-10 text-white relative">
              <button 
                ...
                className="absolute top-8 right-8 p-2 bg-white/10 rounded-full hover:bg-white/20 transition-all no-print"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <button 
                onClick={() => generateReviewPill(true)}
                disabled={pillLoading}
                className="mt-6 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-[11px] font-bold transition-all flex items-center gap-2 no-print"
              >
                {pillLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                Regerar Pílula
              </button>
            </div>
            ...
            <div className="p-8 border-t border-[#E9EAF2] bg-[#F8F9FE] flex gap-4 no-print">
               <button 
                 onClick={() => window.open(`/api/pdf/export?examId=${id}&type=pill`, '_blank')}
      ...
      <div className="flex gap-8">
        ...
        {/* Right Side Panel */}
        <div className="w-[380px] shrink-0 sticky top-6 h-fit space-y-6 no-print">
